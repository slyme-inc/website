...
function ServicesCards() {
  ...
  return (
    <>
      {/* On mobile: Each card takes full screen */}
      <div className="md:hidden">
        {services.map((service) => (
          <section 
            key={service.title}
            id={`services-${service.title.toLowerCase().replace(' ', '-')}`} 
          ...
          >
            ...
            <div className="main-container h-full flex items-center justify-center p-6">
              <div className="glass-effect card-hover rounded-2xl p-8 border border-white/10 flex flex-col items-center w-full transition-all duration-300">
                {/* Icon */}
                <div className={`${
                ...
                {/* Description */}
                <p className="text-white/90 text-left font-cooper mb-6 w-full">
                  {service.description}
                </p>

                {/* Highlights */}
                <ul className="space-y-3 text-white/80 font-cooper self-start">
      ...
      {/* On desktop: Both cards in one screen */}
      <section className="relative h-screen snap-start snap-always hidden md:block">
        ...
        {/* Content */}
        <div className="main-container h-full flex items-center justify-center">
          <div className="grid grid-cols-2 gap-8 w-full max-w-5xl">
            {services.map((service) => (
              ...
              >
                ...
                {/* Description */}
                <p className="text-white/90 text-center md:text-left font-cooper mb-6 w-full">
                  {service.description}
                </p>

                {/* Highlights */}
                <ul className="space-y-2 text-white/80 font-cooper self-start">
                  {service.highlights.map((highlight, i) => (
                    <li key={i} className="flex items-center">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#97ca4d] mr-2"></span>
                      {highlight}
